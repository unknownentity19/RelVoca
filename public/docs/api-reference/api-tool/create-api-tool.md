...
````yaml /specs-prettified/realtime/openapi.stable.json post /v1/stable/api-tool
openapi: 3.0.0
info:
  title: Realtime
...
paths:
  /v1/stable/api-tool:
    post:
      ...
      description: >-
        Adds a API tool to the environment, a saved HTTP request an agent can
        make. The response carries the ID that later calls address it by.
      ...
      responses:
        '201':
          description: >-
            Creates the API tool and returns it in full, including its new ID
            and the request it will send: HTTP method, URL, headers, query
            parameters, and body.
          content:
            application/json:
              schema:
