...
````yaml /specs-prettified/realtime/openapi.stable.json post /v1/stable/document/table
openapi: 3.0.0
info:
  title: Realtime
...
tags: []
paths:
  /v1/stable/document/table:
    post:
      tags:
        - Document
      summary: Create table document
      description: >-
      ...
      requestBody:
        ...
        content:
          application/json:
            schema:
              $ref: '#/components/schemas/StableDocumentCreateTableRequest'
      responses:
        '201':
          description: >-
            Returns the document created from the structured data, with its
            assigned ID and status. The chunks it was split into are not in this
            response.
          content:
            application/json:
              schema:
          ...
          x-vf-doc:
            description: >-
              Returns the document created from the structured data, with its
              assigned ID and status. The chunks it was split into are not in
              this response.
      security:
        - token: []
components:
  schemas:
    StableDocumentCreateTableRequest:
      type: object
      properties:
        url:
          description: An optional source URL to associate with the table document.
          type: string
        name:
          type: string
        items:
          ...
          description: >-
            The table rows to ingest; each row is a flat object and becomes its
            own chunk.
        schema:
          type: object
          properties:
            metadataFields:
              default: []
              description: >-
                Per-row metadata, returned with results. NOT consulted by the
                default system tool's filter UI.
              type: array
              items:
                type: string
            searchableFields:
              ...
              items:
                type: string
              description: Full-text indexed; what semantic search matches against.
          required:
            - searchableFields
          description: >-
            Declares which top-level keys of each row are full-text searchable
            versus row-level metadata.
        metadata:
          default: []
          description: >-
            Properties that can be filtered on at runtime (static or dynamic
            from a variable). Put your most common filter dimensions here.
          type: array
          items:
            type: object
            properties:
