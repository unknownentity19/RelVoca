...
````yaml /specs-prettified/realtime/openapi.stable.json get /v1/stable/evaluation/{evaluationID}
openapi: 3.0.0
info:
  title: Realtime
...
paths:
  /v1/stable/evaluation/{evaluationID}:
    get:
      ...
      description: >-
        Returns a single evaluation by ID, with the fields that define it. An
        evaluation is a criterion a model scores past conversations against.
      operationId: StableEvaluationController_get
      parameters:
        - name: evaluationID
          required: true
          in: path
      ...
      responses:
        '200':
          description: >-
            Returns one evaluation in full: the criteria prompts for its result
            type, the model settings the evaluator judges with, and whether it
            is one of the built-in defaults.
          content:
            application/json:
              schema:
          ...
          x-vf-doc:
            description: >-
              Returns one evaluation in full: the criteria prompts for its
              result type, the model settings the evaluator judges with, and
              whether it is one of the built-in defaults.
      security:
        - token: []
components:
  schemas:
