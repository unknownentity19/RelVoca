...
````yaml /specs-prettified/realtime/openapi.stable.json get /v1/stable/evaluation
openapi: 3.0.0
info:
  title: Realtime
...
paths:
  /v1/stable/evaluation:
    get:
      ...
      description: >-
        Returns every evaluation in the project. An evaluation is a criterion a
        model scores past conversations against. The call takes no paging
        parameters, so one request yields the whole set.
      operationId: StableEvaluationController_list
      parameters:
        - name: projectID
          required: true
          in: query
      ...
      responses:
        '200':
          description: >-
            Returns every evaluation defined on the project, each with the
            criteria it judges transcripts against, its result type, whether it
            runs automatically on new transcripts, and its average cost per run.
          content:
            application/json:
              schema:
                $ref: '#/components/schemas/StableEvaluationListResponse'
          x-vf-doc:
            description: >-
              Returns every evaluation defined on the project, each with the
              criteria it judges transcripts against, its result type, whether
              it runs automatically on new transcripts, and its average cost per
              run.
      security:
        - token: []
components:
  schemas:
    StableEvaluationListResponse:
      type: object
      properties:
        evaluations:
          type: array
