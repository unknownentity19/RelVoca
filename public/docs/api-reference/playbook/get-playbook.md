...
````yaml /specs-prettified/realtime/openapi.stable.json get /v1/stable/playbook/{playbookID}
openapi: 3.0.0
info:
  title: Realtime
...
paths:
  /v1/stable/playbook/{playbookID}:
    get:
      ...
      description: >-
        Returns a single playbook by ID, with the fields that define it. A
        playbook is a set of instructions that drives an agent toward a goal,
        calling tools as it needs them.
      ...
      responses:
        '200':
          description: >-
            Returns a single playbook, including its instructions, its tool
            ordering, and the system tool settings configured on it.
          content:
            application/json:
              schema:
                $ref: '#/components/schemas/StablePlaybookResponse'
          x-vf-doc:
            description: >-
              Returns a single playbook, including its instructions, its tool
              ordering, and the system tool settings configured on it.
