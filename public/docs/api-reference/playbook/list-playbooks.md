...
````yaml /specs-prettified/realtime/openapi.stable.json get /v1/stable/playbook
openapi: 3.0.0
info:
  title: Realtime
...
paths:
  /v1/stable/playbook:
    get:
      ...
      description: >-
        Returns every playbook in the environment. A playbook is a set of
        instructions that drives an agent toward a goal, calling tools as it
        needs them. The call takes no paging parameters, so one request yields
        the whole set.
      ...
      responses:
        '200':
          description: >-
            Returns every playbook in the project environment, each with its
            name, description, instructions, model settings, and its own system
            tool configuration.
          content:
            application/json:
              schema:
          ...
          x-vf-doc:
            description: >-
              Returns every playbook in the project environment, each with its
              name, description, instructions, model settings, and its own
