...
````yaml /specs-prettified/realtime/openapi.stable.json post /v1/stable/test-run
openapi: 3.0.0
info:
  title: Realtime
...
paths:
  /v1/stable/test-run:
    post:
      ...
      summary: Create run
      description: >-
        Adds a test run to the environment, one execution of a test, carrying
        how many of its checks passed. The response carries the ID that later
        calls address it by.
      operationId: StableTestRunController_create
      parameters:
        - name: projectID
          required: true
          in: query
      ...
      requestBody:
        ...
        content:
          application/json:
            schema:
              $ref: '#/components/schemas/StableTestRunCreateRequest'
      responses:
        '201':
          description: >-
            Creates a run of the test named in the body and returns it, possibly
            still queued: `startedAt` is null until execution begins and
            `passedChecks` is null until the run finishes. Read the run back by
            `id` to follow it through to `passed`, `failed`, or `canceled`.
          content:
            application/json:
              schema:
          ...
          x-vf-doc:
            description: >-
              Creates a run of the test named in the body and returns it,
              possibly still queued: `startedAt` is null until execution begins
              and `passedChecks` is null until the run finishes. Read the run
              back by `id` to follow it through to `passed`, `failed`, or
              `canceled`.
      security:
        - token: []
components:
  schemas:
    StableTestRunCreateRequest:
      type: object
      properties:
        testID:
          type: string
